      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {filtered.map((csr) => {
            ...
            // The `timeline` column is rendered as newsletter entries — short, scannable updates.
            ...
            if (entries.length > 0) {
              return (
                <article
                  key={csr.id}
                  id={csr.slug}
                  className="md:col-span-2 lg:col-span-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 sm:p-8"
                >
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <h3 className="font-bold font-heading text-slate-900 dark:text-white text-lg leading-tight">
                      {csr.name}
                    </h3>
                    {csr.location && (
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {entries.map((entry, i) => (
                      <div
                        key={i}
                        className="h-full flex flex-col rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 p-4"
                      >
