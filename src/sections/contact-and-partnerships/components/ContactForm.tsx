...
import { ChevronDown, Check, AlertCircle, Send, Paperclip } from 'lucide-react'
import type {
  FormConfig,
  InquiryType,
  TimelineOption,
...
export function ContactForm({
  ...
  if (submitted) {
    return (
      <div className="h-full flex items-center">
        <div className="p-8 sm:p-10 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 text-center w-full">
          ...
          <p className="text-slate-300/70 leading-relaxed mb-6 max-w-md mx-auto">
          ...
          <ul className="text-left max-w-sm mx-auto space-y-3 mb-6">
            {formConfig.successMessage.nextSteps.map((step, i) => (
              <li key={i} className="flex items-start gap-3">
                <span className="flex-shrink-0 w-5 h-5 rounded-full bg-blue-500/15 border border-blue-400/20 flex items-center justify-center mt-0.5">
                  <span className="text-[10px] font-mono font-bold text-blue-300">{i + 1}</span>
                </span>
                <span className="text-sm text-slate-300/70">{step}</span>
              </li>
            ))}
          </ul>
        </div>
  ...
  const renderField = (
    ...
    const sharedClasses = `w-full px-4 py-3 text-sm rounded-xl bg-white/5 backdrop-blur-md border transition-all duration-300 outline-none font-body text-white placeholder:text-slate-500 ${
      error
        ? 'border-red-400/50 focus:border-red-400 shadow-[0_0_0_1px_rgba(248,113,113,0.15)]'
        : 'border-white/10 focus:border-blue-400/50 focus:shadow-[0_0_0_1px_rgba(59,130,246,0.2)] hover:border-white/15'
    }`
  ...
  return (
    <form onSubmit={handleSubmit} className="h-full">
      <div className="p-6 sm:p-8 lg:p-10 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.08),0_0_0_1px_rgba(255,255,255,0.03)]">
        {/* Form header */}
        <div className="mb-8">
          <p className="text-xs font-mono uppercase tracking-widest text-blue-300/70 mb-3">
            Start a Partnership
          </p>
          ...
          <p className="text-sm text-slate-400 leading-relaxed">
            ...
            <span className="text-blue-300">*</span> are required.
          </p>
        </div>
        ...
        <div className="space-y-4">
          {formConfig.basicFields.map((field) => {
            ...
            return (
              <div key={field.name}>
                <label className="block text-sm font-medium text-slate-300 mb-1.5 font-body">
                  {field.label}
                  {field.required && <span className="text-blue-300 ml-1">*</span>}
                </label>
        ...
        <div className="mt-6">
          <button
            ...
            className="flex items-center gap-2 text-sm text-slate-400 hover:text-blue-300 transition-colors duration-200 group"
          >
            <div
              className={`w-5 h-5 rounded-full border transition-all duration-300 flex items-center justify-center ${
                showDetailed
                  ? 'border-blue-400/50 bg-blue-500/20'
                  : 'border-white/15 bg-white/5 group-hover:border-blue-400/30'
              }`}
            >
              <ChevronDown
                className={`w-3 h-3 transition-transform duration-300 ${
                  showDetailed ? 'rotate-180 text-blue-300' : 'text-slate-500'
                }`}
              />
            </div>
